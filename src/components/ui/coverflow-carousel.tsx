...
export interface CoverflowCarouselProps {
  ...
  label?: string;
  /** Card centred on mount, and re-centred when the selection changes elsewhere. */
  defaultIndex?: number;
  /** Fires when the centred card changes, so parents can sync their own state. */
...
export function CoverflowCarousel({
  ...
  const posRef = React.useRef(defaultIndex ?? 0);
  /** Where the current settle is headed. Stepping off `pos` instead would
      swallow a keypress that lands mid-flight, before the round-off moves. */
  const targetRef = React.useRef(defaultIndex ?? 0);
  ...
  const changeRef = React.useRef(onSlideChange);
  changeRef.current = onSlideChange;

  const [selected, setSelectedState] = React.useState(() =>
    count ? (((defaultIndex ?? 0) % count) + count) % count : 0,
  );
  ...
  const settle = React.useCallback(
    ...
    [indexAt, paint, reduced, setSelected],
  );
  ...
  // Re-centre when the parent selects an episode somewhere else.
  React.useEffect(() => {
    if (defaultIndex == null || !count || dragRef.current) return;
    const folded = ((defaultIndex % count) + count) % count;
    if (indexAt(targetRef.current) !== folded) goTo(folded);
  }, [defaultIndex, count, goTo, indexAt]);
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    dragRef.current = {
      ...
      v: 0,
      t: performance.now(),
      index: (event.target as HTMLElement).closest<HTMLElement>('[data-slide-index]')?.dataset.slideIndex != null
        ? Number((event.target as HTMLElement).closest<HTMLElement>('[data-slide-index]')?.dataset.slideIndex) : null,
    };
  };
  ...
  const endDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (event.type !== 'pointercancel' && Math.abs(event.clientX - drag.x) < 6 && drag.index !== null) {
      goTo(drag.index);
      return;
    }
    // Let a flick carry, but never more than two cards.
    const carried = reduced ? 0 : Math.max(-2, Math.min(2, drag.v * 0.18));
        ...
        >
          ...
          >
            {slides.map((slide, index) => (
              ...
              >
                {renderSlide ? renderSlide(slide, index) : <img
                  src={slide.src}
                  alt={slide.alt}
                  draggable={false}
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          {slides.map((_, index) => (
            <button
              key={index}
              type="button"
              aria-label={`Select ${slides[index].alt}`}
              aria-current={index === selected}
              onClick={() => goTo(index)}
              className={cn(

...
export const Text3DFlip = ({
  ...
  const charOffsets = useMemo(() => {
    ...
    for (const word of characters) {
      offsets.push((offsets[offsets.length - 1] ?? 0) + word.characters.length);
    }
    return offsets;
  }, [characters]);
  ...
  return (
    <ElementTag
      id={id}
      className={cn("relative inline-flex flex-wrap cursor-pointer select-none perspective-1000", className)}
...
const CharBox = memo(
  ({
    ...
    <span
      className="text-3d-flip-char inline-block transform-3d transition-transform duration-75"
